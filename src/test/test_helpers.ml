...
let typed_program_with_imperative_input_to_michelson ~raise ((program , env): Ast_typed.module_fully_typed * Ast_typed.environment) (entry_point: string) (input: Ast_imperative.expression) : Stacking.compiled_expression *  Ast_aggregated.type_expression =
  ...
  let aggregated = Ligo_compile.Of_typed.compile_expression_in_context typed_app program in
  let mini_c = Ligo_compile.Of_aggregated.compile_expression ~raise aggregated in
...
let run_typed_program_with_imperative_input ~raise ?options ((program, env): Ast_typed.module_fully_typed * Ast_typed.environment ) (entry_point: string) (input: Ast_imperative.expression) : Ast_core.expression =
  let michelson_program,ty = typed_program_with_imperative_input_to_michelson ~raise (program, env) entry_point input in
