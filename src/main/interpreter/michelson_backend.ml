...
let compile_contract ~raise ~add_warning ~module_resolutions ~protocol_version source_file entry_point declared_views =
  let open Ligo_compile in
  let syntax = "auto" in
  let options = Compiler_options.make ~protocol_version () in
  let michelson,env = Build.build_contract ~raise ~add_warning ~module_resolutions ~options syntax entry_point source_file in
  let views = Build.build_views ~raise ~add_warning ~module_resolutions ~options syntax entry_point (declared_views,env) source_file in
