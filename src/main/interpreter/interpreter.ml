...
let rec apply_operator ~raise ~steps ~protocol_version ~options : Location.t -> calltrace -> Ast_typed.type_expression -> env -> Ast_typed.constant' -> (value * Ast_typed.type_expression) list -> value Monad.t =
...
and eval_ligo ~raise ~steps ~protocol_version ~options : Ast_typed.expression -> calltrace -> env -> value Monad.t
    ...
    let aux : env * env * Tezos_state.context -> declaration location_wrap -> env * env * Tezos_state.context =
      fun (top_env,curr_env,state) el ->
        ...
        | Ast_typed.Declaration_constant {binder; expr ; attr = { inline=_ ; no_mutation }} ->
          let (v,state) = try_eval ~raise ~steps ~protocol_version ~options expr top_env state None in
