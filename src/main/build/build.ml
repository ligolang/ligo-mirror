...
let build_expression ~raise ~add_warning : options:Compiler_options.t -> string -> string -> file_name option -> _ =
  fun ~options syntax expression file_name ->
    let (aggregated_prg,env) =
      match file_name with
      | Some init_file ->
         let contract, env = combined_contract ~raise ~add_warning ~options syntax init_file in
         let contract = Ligo_compile.Of_typed.compile_program ~raise contract in
         (contract, env)
      | None -> ((fun x -> Ligo_compile.Of_typed.compile_expression ~raise x), options.init_env)
    in
    let typed_exp,_     = Ligo_compile.Utils.type_expression ~raise ~options file_name syntax expression env in
    let aggregated      = Ligo_compile.Of_typed.compile_expression_in_context typed_exp aggregated_prg in
...
let build_contract ~raise ~add_warning : options:Compiler_options.t -> string -> string -> file_name -> Stacking.compiled_expression * Ast_typed.environment =
  fun ~options syntax entry_point file_name ->
    ...
    let aggregated = Ligo_compile.Of_typed.apply_to_entrypoint ~raise (typed_prg, env) entry_point in
...
let build_views ~raise ~add_warning :
  ...
  fun ~options syntax main_name (declared_views,env) source_file ->
    ...
    in
    let f view_name =
      let typed_prg,env  = build_typed ~raise ~add_warning:(fun _ -> ()) ~options syntax (Ligo_compile.Of_core.View (main_name,view_name)) source_file in
      let aggregated = Ligo_compile.Of_typed.apply_to_entrypoint_view ~raise (typed_prg, env) view_name in
      let mini_c = Ligo_compile.Of_aggregated.compile_expression ~raise aggregated in
...
let build_context ~raise ~add_warning :
  options:Compiler_options.t -> string -> file_name -> Ast_typed.module_fully_typed * Ast_typed.environment =
    fun ~options _syntax file_name ->
      let open Build(struct
        let raise = raise
        let add_warning = add_warning
