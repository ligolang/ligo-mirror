...
let%expect_test "typer" =
  ...
  let location_t = File default_location in
  let context = Checking.Context.empty in
  ...
  error (`Typer_unbound_type_variable (context, type_variable, location_t)) ;
  [%expect
    {|
    File "a dummy file name", line 20, character 5:
    ...
    Type "foo" not found.|}] ;
  error
    (`Typer_unbound_variable (context, expression_variable, location_t)) ;
  [%expect
    {|
      File "a dummy file name", line 20, character 5:
  ...
  error
    (`Typer_unbound_constructor
      (context, Label "Some-Constructor", location_t)) ;
  [%expect
    {|
    File "a dummy file name", line 20, character 5:
