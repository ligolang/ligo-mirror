...
let t_signature  ?loc () : type_expression = t_constant ?loc signature_name []
let t_chain_id   ?loc () : type_expression = t_constant ?loc chain_id_name []
let t_string     ?loc () : type_expression = t_constant ?loc string_name []
let t_bytes      ?loc () : type_expression = t_constant ?loc bytes_name []
let t_key        ?loc () : type_expression = t_constant ?loc key_name []
let t_key_hash   ?loc () : type_expression = t_constant ?loc key_hash_name []
let t_int        ?loc () : type_expression = t_constant ?loc int_name []
let t_address    ?loc () : type_expression = t_constant ?loc address_name []
let t_operation  ?loc () : type_expression = t_constant ?loc operation_name []
let t_nat        ?loc () : type_expression = t_constant ?loc nat_name []
let t_mutez      ?loc () : type_expression = t_constant ?loc tez_name []
let t_timestamp  ?loc () : type_expression = t_constant ?loc timestamp_name []
let t_unit       ?loc () : type_expression = t_constant ?loc unit_name []
let t_bls12_381_g1 ?loc () : type_expression = t_constant ?loc bls12_381_g1_name []
let t_bls12_381_g2 ?loc () : type_expression = t_constant ?loc bls12_381_g2_name []
let t_bls12_381_fr ?loc () : type_expression = t_constant ?loc bls12_381_fr_name []
let t_never       ?loc () : type_expression = t_constant ?loc never_name []
let t_pvss_key ?loc () : type_expression = t_constant ?loc pvss_key_name []
let t_baker_hash ?loc () : type_expression = t_constant ?loc baker_hash_name []
let t_chest_key ?loc () : type_expression = t_constant ?loc chest_key_name []
let t_chest ?loc () : type_expression = t_constant ?loc chest_name []

(*
let t_abstraction1 ?loc name kind : type_expression = 
  let ty_binder = Location.wrap @@ Var.fresh () in
  let type_ = t_constant name [t_variable :(Ast.t_variable ty_binder.wrap_content) ty_binder.wrap_content] in
...
let t_option         ?loc o   : type_expression = t_constant ?loc option_name [o]
let t_list           ?loc t   : type_expression = t_constant ?loc list_name [t]
let t_set            ?loc t   : type_expression = t_constant ?loc set_name [t]
let t_contract       ?loc t   : type_expression = t_constant ?loc contract_name [t]
let t_typed_address  ?loc p s : type_expression = t_constant ?loc typed_address_name [ p ; s ]
let t_mutation       ?loc ()  : type_expression = t_constant ?loc mutation_name []
let t_failure        ?loc ()  : type_expression = t_constant ?loc failure_name []
let t_ticket         ?loc t   : type_expression = t_constant ?loc ticket_name [t]
let t_map            ?loc k v : type_expression = t_constant ?loc map_name [ k ; v ]
let t_big_map        ?loc k v : type_expression = t_constant ?loc big_map_name [ k ; v ]
let t_map_or_big_map ?loc k v : type_expression = t_constant ?loc map_or_big_map_name [ k ; v ]
