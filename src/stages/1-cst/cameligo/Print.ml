...
and print_declaration state = function
  Let {value = (_kwd_let, kwd_rec, let_binding, attr); region} ->
    ...
    (if kwd_rec <> None then print_node (state#pad 0 0) "rec"); (* Hack *)
...
and print_pattern state = function
  PConstr p ->
    ...
    print_ctor_pattern (state#pad 1 0) p
...
and print_ctor_pattern state {value; _} =
...
| EConstr e_ctor ->
    ...
    print_ctor_expr (state#pad 1 0) e_ctor
...
| EModPath p ->
    print_module_path state "EModPath" print_expr p
...
| TApp {value; region} ->
    ...
    print_module_path   (state#pad 2 0) "<constructor>" print_ident mod_path;
    print_type_ctor_arg (state#pad 2 1) type_ctor_arg
| TFun {value; region} ->
...
| TModPath p ->
    print_module_path state "TModPath" print_ident p
| TArg t ->
