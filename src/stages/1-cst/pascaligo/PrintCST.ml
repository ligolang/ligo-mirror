...
let rec print_cst state (node : cst) =
  print_list state "<cst>" print_declaration
...
and print_param_decl state (node : param_decl reg) =
  ...
  let children = [
    ...
    Tree.mk_child     print_variable        value.var;
...
and print_declarations state (node : declarations) =
  print_list state "<structure>" print_declaration
...
and print_T_Sum state (node : sum_type reg) =
  ...
  let attributes =
    [Tree.mk_child_list print_attributes node.attributes]
  in Tree.print state "T_Sum" (variants @ attributes)

and print_variant state (node : variant reg) =
  let node     = node.value in
  let children = [
    Tree.mk_child_opt  print_of_type_expr node.args;
...
and print_I_ForIn state (node : for_in reg) =
  ...
  let print_var_binding state (source, image) =
    ...
    in Tree.print state "<binding>" children in
...
and print_map_lookup state label (node : map_lookup reg) =
  ...
  and print_index state (index : expr brackets reg) =
    Tree.print_unary state "<index>" print_expr index.value.inside in

  let children = [
    Tree.mk_child print_map   value.map;
    Tree.mk_child print_index value.index]
...
and print_P_Nil state wrap =
  Tree.print_node ~region:wrap#region state "P_Nil"
...
and print_field_pattern state (node : field_pattern reg) =
  let print_lhs state =
    Tree.print_unary state "<lhs>" Tree.print_literal
  and print_rhs state =
  ...
  fun state ~lhs:print_lhs ~rhs:print_rhs {value; region} ->
    match value with
      Punned {pun; attributes} ->
        let children = [
          ...
          Tree.mk_child_list print_attributes attributes] in
    ...
    | Complete {field_lhs; field_rhs; attributes; _} ->
        let children = [
          ...
          Tree.mk_child_list print_attributes attributes]
